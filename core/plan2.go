...
func FindMetaInfo(c *Coordinator, tablename string) (TableMeta, Partition, error) {
	var table_meta TableMeta
	var partition_meta Partition
	var err error
	// find metaData
	is_find_ := false
	for _, element := range c.table_metas.TableMetas {
	...
	for _, element := range c.partitions.Partitions {
		if strings.EqualFold(element.TableName, tablename) {
...
func HandleCreateTable(c *Coordinator, stmt ast.StmtNode) ([]SqlRouter, error) {
	...
	// find meta
	table_meta, partition_meta, err := FindMetaInfo(c, tablename)
	if err != nil {
		return ret, err
	}
	if strings.EqualFold(partition_meta.FragType, "HORIZONTAL") {
...
func HandleDropTable(c *Coordinator, stmt ast.StmtNode) ([]SqlRouter, error) {
	// router the create table sql to partitioned site
	// Horizontal fragmentation only
	// only consider one fragmentation condition
	var ret []SqlRouter
...
func HandleInsert(c *Coordinator, stmt ast.StmtNode) ([]SqlRouter, error) {
...
func HandleDelete(c *Coordinator, stmt ast.StmtNode) ([]SqlRouter, error) {
...
func BroadcastSQL(c *Coordinator, stmt ast.StmtNode) ([]SqlRouter, error) {
	var ret []SqlRouter
	sql := stmt.Text()
	fmt.Println(sql)
...
func HandleSelect(c *Coordinator, sel *ast.SelectStmt) (p BasePlan, err error) {
	...
	if sel.From != nil {
		p, err = buildResultSetNode(c, sel.From.TableRefs)
		if err != nil {
			return nil, err
		}
	} else {
	...
	if sel.Where != nil {
		p, err = buildSelection(c, p, sel.Where)
		if err != nil {
			return nil, err
		}
	}

	p, err = buildProjection(c, p, sel.Fields.Fields)
	if err != nil {
		return nil, err
	}
...
func buildProjection(c *Coordinator, p BasePlan, fields []*ast.SelectField) (BasePlan, error) {
	proj := PlanTreeNode{
		Type: ProjectionType,
	}.Init()
...
func buildSelection(c *Coordinator, p BasePlan, where ast.ExprNode) (BasePlan, error) {
...
func buildJoin(c *Coordinator, joinNode *ast.Join) (BasePlan, error) {
	if joinNode.Right == nil {
		return buildResultSetNode(c, joinNode.Left)
	}

	leftPlan, err := buildResultSetNode(c, joinNode.Left)
	if err != nil {
		return nil, err
	}

	rightPlan, err := buildResultSetNode(c, joinNode.Right)
	if err != nil {
		return nil, err
	}
...
func buildDataSource(c *Coordinator, tn *ast.TableName) (BasePlan, error) {
	// dbName := tn.Schema
...
func buildResultSetNode(c *Coordinator, node ast.ResultSetNode) (p BasePlan, err error) {
	switch x := node.(type) {
	case *ast.Join:
		return buildJoin(c, x)
	case *ast.TableSource:
		switch v := x.Source.(type) {
		case *ast.TableName:
			p, err = buildDataSource(c, v)
		default:
...
func ParseAndExecute(c *Coordinator, sql_str string) (BasePlan, []SqlRouter, error) {
	var p BasePlan
	var ret []SqlRouter
	var err error
